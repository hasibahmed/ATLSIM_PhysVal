...
bool is_norm = true; 
std::string leg1 = "G4 thres. 0";
std::string leg2 = "G4 thres. 32";
std::string leg3 = "G4 thres. 64";
std::string leg4 = ""; 
std::string xlabel = "";
...
TCanvas* compare_TProfile(TKey *key)
{
    ...
    if (file4)
        hist4 = (TProfile *)file4->Get(h_name);
     ...
     if (hist4)
    {
        ...
        hist4->SetMarkerStyle(kOpenCircle);
    ...
    hist_ratio_1_2->GetYaxis()->SetRangeUser(0.9, 1.1); 
    ...
    TProfile *hist_ratio_1_4 = nullptr;
    if (hist4)
    {

        hist_ratio_1_4 = (TProfile *)hist4->Clone();
        hist_ratio_1_4->Divide(hist);
        hist_ratio_1_4->SetLineColor(kBlue);
        hist_ratio_1_4->SetMarkerStyle(kOpenCircle);
        hist_ratio_1_4->SetMarkerColor(kBlue);
    ...
    if (hist4)
        leg->AddEntry(hist4, leg4.c_str(), "lpe");
...
TCanvas* compare_TH1F(TKey *key)
{
    ...
    TH1F *hist_ratio_1_4 = nullptr;
    if (hist4)
    {

        hist_ratio_1_4 = (TH1F *)hist4->Clone();
        hist_ratio_1_4->Divide(hist);
        hist_ratio_1_4->SetLineColor(kGreen);
        hist_ratio_1_4->SetMarkerStyle(kOpenCircle);
        hist_ratio_1_4->SetMarkerColor(kGreen);
    }
...
void runLowEnergyValidation(std::string object="Jets", bool isNorm = false, std::string outDir="./ImpactOfLowEnergy/", std::string inDir="./")
{
    ...
    is_norm = isNorm; 
    std::vector<std::string> v_samples = {"ttbar"}; 
    ...
    for(int i = 0; i < v_samples.size(); i++) {
        ...
        // std::map<std::string, std::string> v_files =  
        
        
        // loadInputs(inDir, sample); 
        ...
        TFile* f = TFile::Open("LowEnergyThresholdSamples/physval_Tthr0.pool.root"); 
        TFile* f2 = TFile::Open("LowEnergyThresholdSamples/physval_Tthr32.pool.root"); 
        // TFile* f3 = nullptr; 
        TFile* f3 = TFile::Open("LowEnergyThresholdSamples/physval_Tthr64.pool.root");
        TFile* f4 = nullptr; 
        ...
        for(int j = 0; j < v_dir.size(); j++) {
            ...
            if(f4)
                file4 = (TDirectoryFile*)f4->Get(dir.c_str());
